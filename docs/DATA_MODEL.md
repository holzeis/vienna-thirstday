...
live in `backend/drizzle/` (current head: `0012_fancy_jazinda.sql`). This document
...
erDiagram
  ...
  users ||--o{ password_reset_tokens : "generated by"
```
...
`invites.used_by_user_id`, `player_merges.merged_by_user_id`,
`password_reset_tokens.created_by_user_id` — is nullable
...
**`password_reset_tokens`** — an admin-generated, single-use password-reset
link for a user who forgot theirs. At most one row per user: generating a
new link deletes any previous one for that user first (see
`docs/ARCHITECTURE.md` #10), so an old link stops working the moment a new
one is issued, not just on use or expiry.

| Column | Type | Notes |
| --- | --- | --- |
| `id` | serial | PK |
| `token` | varchar(64) | unique |
| `user_id` | integer | FK → `users.id`, cascade delete — meaningless once the user is gone |
| `expires_at` | timestamptz | 1 hour after creation |
| `used_at` | timestamptz | nullable |
| `created_by_user_id` | integer | FK → `users.id`, nullable, `ON DELETE SET NULL` — the admin who generated it |
| `created_at` | timestamptz | |
